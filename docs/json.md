...
Structurally, it can be useful to think of JSON as being analogous to XML, but without widely standardized related facilities such as DTDs, schemas, XSL transforms, and XPath, and with an anonymous root element. (That may sound very limiting; it actually isn't as limiting as it might appear, and the benefit---a reduction in the verbosity that is a common pain point for XML users---often justifies it.)
